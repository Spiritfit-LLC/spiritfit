<div class="personal-reg <?if ($arResult['PROFILE_PAGE']) echo '-personal'; else echo '-modal'?>">
    <div class="personal-reg-header -modal-header">
        <div class="personal-reg-title">
    ...
    <div class="personal-reg-body <?if ($arResult['PROFILE_PAGE']) echo '-personal-body'; else echo '-modal-body'?>">
        <form class="personal-reg-form" action="<?=$arResult['ajax']?>" method="post" enctype="multipart/form-data">
            <input type="hidden" name="STEP" value="1">
            <?foreach($arResult['FORM_FIELDS'][$arParams['REGISTER_FORM_CODE']]['HIDDEN'] as $FIELD):?>
                <input
                    name="<?=$FIELD['NAME']?>"
                    type="<?=$FIELD['TYPE']?>"
                    value="<?=$FIELD['VALUE']?>"
            ...
            <div class="personal-reg-form-data ">
                <?foreach($arResult['FORM_FIELDS'][$arParams['REGISTER_FORM_CODE']]['VISIBLE'] as $FIELD):?>
                    ...
                    <?else:?>
                        ...
                        <input
                            ...
                            value="<?=$FIELD['VALUE']?>"
                            <? if ($FIELD["REQUIRED"]) { ?>required="required"<? } ?>
                            <? if ($FIELD['VALIDATOR']['LENGTH_FROM']):?> minlength="<?=$FIELD['VALIDATOR']['LENGTH_FROM']?>" <?endif;?>
                            <? if ($FIELD['VALIDATOR']['LENGTH_TO']):?> maxlength="<?=$FIELD['VALIDATOR']['LENGTH_TO']?>" <?endif;?>
                        >
                    <?endif;?>
                    <?if ($FIELD["COMMENT"]):?>
            ...
            <input class="personal-reg-form-btn button-outline -modal-btn" type="submit" value="ОТПРАВИТЬ">
