...
$textDescription = $arResult['PROPERTIES']['BLOCK_TEXT']['DESCRIPTION'];
						...
						<? if(!empty($text)){ ?>
							<? foreach ($text as $key => $itemText) { ?>
								<div class="<?=$class?>__text-item">
									<h2><a href="<?=(!empty($linkTitle) ? $linkTitle : $link)?>"><?=(!empty($textDescription[$key])) ? $textDescription[$key] : $arResult['NAME']?></a></h2>
									<?=$itemText['TEXT']?>
								</div>
							<? } ?>
						<? } ?>
