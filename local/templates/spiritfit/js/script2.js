...
$(document).ready(function() {  
    ...
    var init = function() {
        ...
        if ($("#map").length) {
            ymaps.ready(function () {
                ...
                if ($("#map").data("abonement") == "Y"){
                    var str = $("#map").data("coord-mark-lat");
                    var cord1 = str.split(',');
                    str = $("#map").data("coord-mark-lng");
                    var cord2 = str.split(',');
        ...
        if(value != ""){
            $.ajax({
                ...
                if(data.errorCode == "0"){
                    ...
                    var oldPrice = $('.subscription__total-value-old span').text();
                    var newPrice = "";
                    if(oldPrice){
	...
	window.formSubscriptionTotal = 1;
    $(document).on('click', '.subscription__total-btn--reg', function() {
        if(window.formSubscriptionTotal) {
        	window.formSubscriptionTotal = 0;
	        var frm = $('form.subscription__aside-form');
            window.asideFormSerialize = frm.serialize();
	        $.post(
	        	'/local/ajax/submit_form.php',
	        	asideFormSerialize + '&web_form_submit=1&web_form_apply=Y&sessid=' + BX.bitrix_sessid() + '&club=' + frm.find('select.js-pjax-select').val()
	        );
	    }
    });
    ...
    $(document).on('click', '.subscription__total-btn--legal-information', function() {
        ...
        $.post(
            '/local/ajax/form_code.php',
            asideFormSerialize,
            function(data) {
                setTimeout(function () {
                    $('.subscription__aside-stage').html(data).show();
                    $('.input--num').on('keyup', function (evt) {
                        if (evt.target.value > 1) {
                            this.value = evt.target.value.slice(0, 1)
                        }
...
/* Grid Main PS */
    ...
    $('.js-submit-abonement').on('click', function(e){
        e.preventDefault();
