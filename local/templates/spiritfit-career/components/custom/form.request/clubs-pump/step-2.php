    ...
    <div class="training__aside-stage" data-stage="1">
        <form class="training__aside-form" name="<?= $arResult["WEB_FORM_NAME"] ?>" action="<?= POST_FORM_ACTION_URI ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="subscription__bottom">
                <div class="subscription__total-btn subscription__total-btn--form btn btn--white js-check-code-training" data-stage="2">Отправить</div>
            </div>
        </form>
    <div>
</div>
...
<script>
    dataLayerSend('conversion', 'sendFormTrialWorkout', '');
    (dataLayer = window.dataLayer || []).push({
        'eCategory': 'UX',
        'eAction': 'openSmsCodePage',
        'eLabel': '5MillionRubbles',
        'eNI': false,
        'event': 'GAEvent'
    });
</script>