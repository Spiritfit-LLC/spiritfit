...
document.addEventListener("DOMContentLoaded", function () {
	...
	let spiritFitnessIcon = iconPath + "map-marker-icon.png";
	let spiritFitnessIconActive = iconPath + "map-marker-icon-active.png";
	let IconAnchor = [20, 47];

	if (window.innerWidth < 800) {
		spiritFitnessIcon = iconPath + "map-marker-icon-small.png";
		spiritFitnessIconActive = iconPath + "map-marker-icon-small-active.png";
		IconAnchor = [12.5, 29];
	}
	...
	function setActiveIcon(marker) {
		...
		if( $(".b-map__info-plate").hasClass("hidden") ) {
			$(".b-map__info-plate").removeClass("hidden");
		}
		...
		let iconNotActive = L.icon({
			iconUrl: spiritFitnessIcon,
			iconAnchor: IconAnchor
		});
	...
	function setInfoContent(marker) {
		...
		if (marker.options.club_soon_open == 'Y' || marker.options.page === '') {
			...
			$titlePlace.html("<a href='" + marker.options.page + "'>" + marker.options.title + "</a>");
			$linkButton.removeClass('is-hide').attr('href', marker.options.page);
			shedule = `<div class="b-map__contact-item"> <a href="${marker.options.page}#timetable">Расписание</a></div>`;
		...
		if (marker.options.description !==undefined &&  marker.options.description.length > 0) {
			shortDescrBlock = `<div class="b-map__contact-item-descr-wrapper"><div class="b-map__contact-item-descr">${marker.options.description}</div>
          	<a class="b-map__contact-item-descr-more" href="#">Подробнее</a></div>`;
		...
		let contactLayout = `${shortDescrBlock}
			...
			</div> <div class="b-map__contact-item"> <div> <a href="tel:${refactoredPhone}"
								class = "invisible-link"
								onclick = "dataLayerSend('UX','clickCallButton', document.location.protocol+'//'+document.location.host+document.location.pathname);">${marker.options.phone}</a></div><div><a href = "mailto:${marker.options.email}"
								class = "invisible-link">${marker.options.email}</a></div></div>
			<div class="b-map__contact-item">${workHoursBlock}</div>${shedule}${min_price}`;
		$contactsPlace.html(contactLayout);
	}

	// функция для построения маршрута
	function setRouteMap(startMarker, endMarker){
	...
	function clubSelectSlider() {
		$("#mapslider").on('init', function (event, slick) { /* --------------------- Отключено --------------------- */
	...
	let fullDescrBlock = `<div class="b-map__contact-full-descr-wrapper" style="display:none"><div class="b-map__contact-full-descr"></div><div class="b-map__contact-full-descr-bg"></div></div>`;
	...
	$('.b-map__info-plate-closer').click(function(){
		$(".b-map__info-plate").fadeOut(300, function (){
