...
class QuizComponent extends CBitrixComponent implements Controllerable {
    ...
    public function executeComponent() {
        ...
        $startDateWeek = date('w', strtotime('-1 week')); //315532800; FOR ALL RESULTS
        $week_start = date('Y-m-d', strtotime('-'.($startDateWeek-1).' days'));

        if ($USER->IsAdmin()){
            $startDateWeek = date('w', strtotime('-1 week')); //315532800; FOR ALL RESULTS
            $week_start = date('Y-m-d', strtotime('-'.($startDateWeek-1).' days -1 week'));
        }
        ...
        $this->arResult['RESULT_TABLE_USER'] = $quiz->getUserResults($startDateAll, ($currentTime - intval($this->arParams['SHOW_RESULT_AFTER'])));

        $this->arResult['EMAIL_WARNING'] = false;
        if( $USER->IsAuthorized() ) {
            $obCache = new CPHPCache();
            if($obCache->InitCache($cacheTime, $cacheHash, $cacheDir) && false) {
                ...
                if( $isShowAlways || ( $showStartTime < $currentTime) ) {
                    $this->arResult['RESULT_TABLE'] = $isShowAlways ? $quiz->getAllResults($startDateAll, $endDateAll) : $quiz->getAllResults(strtotime($week_start), ($currentTime - intval($this->arParams['SHOW_RESULT_AFTER'])));
                }
        ...
        if ($this->arResult["LINK_LOGIN"]){
            Loader::IncludeModule("form");
            $auth_form_id=PersonalUtils::GetIDBySID("AUTH");
            $reg_form_id=PersonalUtils::GetIDBySID("REGISTRATION_LITE");
            $this->arResult['FORM_FIELDS']=[
                "AUTH"=>PersonalUtils::GetFormFileds($auth_form_id, "login", false, "ВОЙТИ"),
                "REG"=>PersonalUtils::GetFormFileds($reg_form_id, "easyregistration", false, "Регистрация"),
            ];
        }
